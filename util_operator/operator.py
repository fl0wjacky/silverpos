...
Builder.load_file('util_operator/operator.kv')
...
class OperatorWindow(BoxLayout):
    ...
    def update_purchases(self):
        ...
        db_product = self.db.stocks.find_one({'product_code':pcode})
        if db_product is None:
            return
        else:
            details = BoxLayout(size_hint_y=None,height=50)#,pos_hint={'top':1})
            products_container.add_widget(details)

            code = Label(text=pcode, size_hint_x=.2,color=(.06,.45,.45,1))
            name = Label(text=db_product['product_name'], size_hint_x=.3,color=(.06,.45,.45,1))
            qty = Label(text='1',size_hint_x = .1,color=(.06,.45,.45,1))
            disc = Label(text='0.00',size_hint_x=.1,color=(.06,.45,.45,1))
            price = Label(text=str(db_product['product_price']),size_hint_x=.1,color=(.06,.45,.45,1))
